...
from myapp.models import Post, User, Comment
from myapp.forms import LoginForm, EditForm, PostForm, SignUpForm, ChangeForm, CommentForm
...
def index(page=1):
	...
	posts = Post.query.order_by(db.desc(Post.time)).paginate(page, 3, False)
...
@app.route('/<index>/add_comment', methods=['POST'])
@login_required
...
@app.route('/<index>/<user_name>/reply', methods=['GET', 'POST'])
@login_required
